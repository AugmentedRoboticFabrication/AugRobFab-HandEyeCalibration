...
class CameraCalibration:
	...
	def __init__(
		...
		"""
		Initializes the CameraCalibration object.
			...
			if ret:
				...
				if ret:
					rVec, t = cv2.solvePnPRefineLM(
						objp, corners2, self.intrinsic_matrix, self.distortion_coefficients, rVec, t)
	...
	def calibrate(self, method=cv2.CALIB_HAND_EYE_TSAI):
		...
		# Read base to TCP transformation data
		file_path = os.path.join(self.dir, "base_T_tcp.json")
		data = readJSON(file_path)

		# Prepare data for calibration and remove failed detections
		...
		# Compose and save the estimated transformation matrix
